...
std::pair<double, double> create_ser_without_resistance(double V_x, double V_y, double g, double t, double x0, double y0)
{
	std::pair<double, double> coord;
	coord.first = x0 + V_x*t;
	coord.second = - y0 - (V_y*t-g*t*t/2);

	return coord;
}


std::pair<double, double> create_numer_approx(const int N, double k, double m, double g, double V_x, double V_y, int i)
{
	std::pair<double, double> coord;
	double t = 0.1;
	double k1 = 0, k2 = 0, k3 = 0, k4 = 0, l1 = 0, l2 = 0, l3 = 0, l4 = 0;
	static double *pY = new double [N]; pY[0] = 0;
	static double *pX = new double [N]; pX[0] = 0;
...
std::pair<double, double> create_relativism(double V_y, double V_x, double m, double k, double g, int N, int j)
{
	std::pair<double, double> coord;
...
std::pair<double, double> create_arbitrary_angle(double &m_p, double k, double &V_xp, double &V_yp,
												 double m_p2, double g, double V_xt, double V_yt,
												 double t_detach, double &x0, double &y0, double t)
{
	std::pair<double, double> coord;
	...
	double dt = 0;
		...
		coord.first = x0 + m_p*V_xp/k*(1 - exp(-k*(t-dt)/m_p));
		coord.second = -(y0 + m_p/k*((V_yp+m_p*g/k)*(1 - exp(-k*(t-dt)/m_p)) - g*(t-dt)));
