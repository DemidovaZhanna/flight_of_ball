...
QLineSeries* MainWindow::create_VV ( double _h, double V_y, double V_x,
											 double m, double k, double g, int N, double x0, double y0)
{
	QLineSeries* series = new QLineSeries();
	series->setName("сила ~ V^2(численное приближение)");
...
{
	QLineSeries* series = new QLineSeries;
	series->setName("случай резкого изменения массы под заданным углом:");
	...
	, ui(new Ui::MainWindow)
{
	...
	chartView->setMinimumSize(t.width() / 2, t.height() / 2);

	chrt = new MyChart(0, 13, 6);
...
//	double V_x1 = 10000000,
//		   V_y1 = 10000000;


	QLineSeries* series4 = create_VV ( dt, V_y, V_x, m, k, g, N, x0, y0);
	...
	chartViw->setMinimumSize(t.width() / 2, t.height() / 2);
	chartViw->setGeometry(t.width() / 2, 0, t.width() / 2, t.height() / 2);

	chart = new MyChart(0, 18, 6);
